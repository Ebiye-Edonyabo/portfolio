    ...
    <div class="col-span-12 lg:col-span-4 bg-[#121212] border border-[#1f1f1f] rounded-xl p-6 h-fit">
        ...
        <form wire:submit="saveExperience" class="space-y-4">
            ...
            <div class="space-y-2" x-data="{ 
                    ...
                    <template x-for="(project, index) in projects" :key="index">
                        <div class="flex gap-2 items-center">
                            <div class="grow grid grid-cols-2 gap-2">
                                <div>
                                    <input type="text" x-model="project.name" required
                                        class="w-full bg-[#171717] border border-[#1f1f1f] text-white px-3 py-2 rounded-lg text-xs outline-none focus:border-primary-300 transition-all"
                                        placeholder="Project Name">
                                    <template x-if="errors['form.projects.' + index + '.name']">
                                        <span class="text-red-400 text-[10px] block mt-0.5" x-text="errors['form.projects.' + index + '.name'][0]"></span>
                                    </template>
                                </div>
                                <div>
                                    <input type="url" x-model="project.url" required
                                        class="w-full bg-[#171717] border border-[#1f1f1f] text-white px-3 py-2 rounded-lg text-xs outline-none focus:border-primary-300 transition-all"
                                        placeholder="Project URL">
                                    <template x-if="errors['form.projects.' + index + '.url']">
                                        <span class="text-red-400 text-[10px] block mt-0.5" x-text="errors['form.projects.' + index + '.url'][0]"></span>
                                    </template>
                                </div>
                            </div>
                            <button type="button" @click="projects.splice(index, 1)" 
                ...
                <button type="button" @click="projects.push({name: '', url: ''})" 
